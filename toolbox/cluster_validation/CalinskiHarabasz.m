function CH = CalinskiHarabasz(X, C, L, weights, Xi, sx, varargin)
% Description: 
% Computes Calinski-Harabasz index value. Three options are available 
...
if ~strcmp(treatmissing,'exp')
    ...
    Inter = nansum(Ni.*nanmatrixdist(C,nancentroid(X,weights),dist2));
    ...
    Inter = nansum(Ni.*nanmatrixdist_exp(C,nancentroid(X,weights,Xi,sx),'sqe'));
    CH = ((N-K)/(K-1))*(Inter/Intra);
    % Inverse
    CH = 1 / CH;
end
