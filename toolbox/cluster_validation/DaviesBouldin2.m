function DB = DaviesBouldin2(X, C, L, weights, Xi, sx, varargin)
% Description: 
% Computes Davies-Bouldin* index value. Three options are available 
...
if ~strcmp(treatmissing,'exp')
    ...
    interD = nandistfunc_exp(C,C,distance);
    interD(logical(eye(size(interD)))) = Inf;
    R = zeros(num);
    cnt = 1;
    for i = 1:num
