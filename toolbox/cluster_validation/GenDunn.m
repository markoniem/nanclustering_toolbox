function GDunn = GenDunn(X, C, L, weights, Xi, sx, varargin)
% Description: 
% Computes Generalized Dunn index value. Three options are available 
...
if ~strcmp(treatmissing,'exp')
    ...
    for i = 1:num
        members = (L == clusts(i));
        Intra(i) = nanmean(nanmatrixdist(X(members,:),C(i,:),distance),weights(members));
    end
    Intra = max(Intra);
    ...
    for i = 1:num
        members = (L == clusts(i));
        Intra(i) = nanmean(nanmatrixdist_exp(Xi(members,:), ...
