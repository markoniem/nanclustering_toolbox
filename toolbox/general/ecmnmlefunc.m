function [Xi, sx, success] = ecmnmlefunc(X, varargin)
% Description: 
% Computes conditional means and variances for data with missing values.
%
% Input:
%         X - Original data set with missing values
% 'Weights' - Weights of input data. Default value: a vector of ones.
%
% Outputs:
%        Xi - Imputed data set using conditional means  
%        sx - Conditional variances of missing values
%   success - Indicator flag for successfully finished algorithm  
%
warning off;
pnames = {'weights'};
dflts =  {ones(size(X,1),1)};
weights = internal.stats.parseArgs(pnames, dflts, varargin{:});
weights = weights(:);
...
Xpopulation = [];
for i = 1:size(X,1)
    Xpopulation = [Xpopulation; repmat(X(i,:),weights(i),1)];
end
if sum(isnan(X(:))) > 0
    [Mu, Covariance, success] = EM(Xpopulation);
    if ~success
        Xi = X;
        sx = zeros(size(X));
        return;
...
% Description: 
% Estimates conditional mean vector and covariance matrix based on expectation 
...
% Description: 
% Uses available data strategy for estimating statistical parameters of 
...
% Description: 
% Divides covariance matrix into distinct parts.
%
% Inputs:
