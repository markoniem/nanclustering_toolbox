function D = nanpdist(Xm, varargin)
% Description:
% Computes pairwise distances between all observations in selected 
% data set. Function assumes that data contains missing values. Available data, 
% partial, expected squared euclidean, and expected euclidean distances 
...
%
D = nandistfunc(Xm, Xm, distance, sx, sx);
N = size(Xm,1);
B = tril(ones(N,N),-1);
D = D(B==1);
