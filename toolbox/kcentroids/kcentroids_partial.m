function [L, C, sumd] = kcentroids_partial(X, k, varargin)
% Description: 
% Perform K-centroids clustering using partial distance strategy for 
% treating missing values. 
%
% Function calls:
% [L, C, sumd] = kcentroids_partial(X, k)
% [L, C, sumd] = kcentroids_partial(X, k, replicates)
% [L, C, sumd] = kcentroids_partial(X, k, replicates, distance)
% [L, C, sumd] = kcentroids_partial(X, k, replicates, distance, initcrit)
% [L, C, sumd] = kcentroids_partial(X, k, replicates, distance, initcrit, start)
%
% Inputs:
%          X - Input data set 
...
% Description: 
% Compute nearest centroids for each observation using City 
% block distance.
%
% Inputs:
%        C - Matrix of cluster centroids
...
while iters < max_iter
    ...
    alpha = size(X,2)./sum(~isnan(X_org),2);
    a = 1./sqrt(sum(D.^2,2).*alpha+sqrt(eps));
    a = bsxfun(@times,P,a);
